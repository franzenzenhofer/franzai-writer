...
- Name: Press Release Workflow >> should generate optional press photo
- Location: /Users/franzenzenhofer/dev/franzai-writer/tests/e2e/press-release-workflow.spec.ts:238:7
...
Call log:
  - waiting for locator('input[placeholder*="New Product Launch"]')
    ...
    at /Users/franzenzenhofer/dev/franzai-writer/tests/e2e/press-release-workflow.spec.ts:242:68
```
...
> 242 |     await page.locator('input[placeholder*="New Product Launch"]').fill('Photo Test');
  ...
  279 |       const imageCount = await images.count();
  280 |       expect(imageCount).toBeGreaterThan(0);
  281 |       
  282 |       console.log(`Photo generation completed with ${imageCount} images`);
  283 |     });
  284 |   });
  285 | });
```