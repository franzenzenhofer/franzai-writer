...
test.describe('Poem Workflow Master Test (Chrome Only)', () => {
  ...
  test('Complete poem workflow with export and persistence', async ({ page }) => {
    ...
    // Verify poem was generated by checking for the poem content
    const poemCard = page.locator('[data-testid="stage-card-generate-poem-with-title"]');
    await expect(poemCard).toContainText('Poem Title');
    await expect(poemCard).toContainText('Poem Content');
    ...
    // Stage 6: HTML Briefing (optional - click to mark complete)
    console.log('📝 Stage 6: Completing optional HTML briefing...');
    await page.waitForSelector('#process-stage-html-briefing', { timeout: 10000 });
    await page.click('#process-stage-html-briefing');
    
    // Wait a moment for dependencies to update
    await page.waitForTimeout(2000);
    
    // Stage 7: HTML Preview (should autorun after poem + image are done)
    console.log('⏳ Stage 7: Waiting for HTML preview...');
    
    // First wait for the stage card to exist
    const htmlPreviewCard = page.locator('[data-testid="stage-card-generate-html-preview"]');
    await htmlPreviewCard.waitFor({ timeout: 10000 });
    
    // Give autorun a chance to trigger
    await page.waitForTimeout(3000);
    
    // Check if the stage is stuck in "Generating..." state or needs manual trigger
    const isGenerating = await page.locator('[data-testid="stage-card-generate-html-preview"]:has-text("Generating...")').isVisible();
    const processButton = page.locator('#process-stage-generate-html-preview');
    const processButtonVisible = await processButton.isVisible();
    
    console.log(`📊 HTML preview state: isGenerating=${isGenerating}, processButtonVisible=${processButtonVisible}`);
    
    if (processButtonVisible && !isGenerating) {
      console.log('📌 HTML preview requires manual trigger...');
      await processButton.click();
      await page.waitForTimeout(2000);
    } else if (isGenerating) {
      console.log('⏳ HTML preview is already generating, waiting for completion...');
    }
    
    // Wait for HTML content to appear - check for multiple possible selectors
    try {
      await page.waitForSelector('[data-testid="stage-card-generate-html-preview"] iframe', { timeout: 30000 });
      console.log('✅ HTML preview generated (iframe found)');
    } catch (e) {
      // If no iframe, check for other content indicators
      const hasContent = await page.locator('[data-testid="stage-card-generate-html-preview"] code').isVisible() ||
                        await page.locator('[data-testid="stage-card-generate-html-preview"] pre').isVisible();
      
      if (!hasContent) {
        // Check if there's an error message
        const errorMsg = await page.locator('[data-testid="stage-card-generate-html-preview"] .text-destructive').textContent();
        if (errorMsg) {
          throw new Error(`HTML preview generation failed with error: ${errorMsg}`);
        }
        throw new Error('HTML preview did not generate any content after 30 seconds');
      }
      console.log('✅ HTML preview generated (code/pre content found)');
    }
    ...
    // Verify document appears in recent docs with poem topic
    await expect(page.locator('text=mountain lake').first()).toBeVisible();
